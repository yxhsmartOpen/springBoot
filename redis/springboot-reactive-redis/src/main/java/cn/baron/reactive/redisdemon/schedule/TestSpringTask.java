package cn.baron.reactive.redisdemon.schedule;
 ...
 *
 * 使用方式二：多线程执行
 * 在传统的Spring项目中，我们可以在xml配置文件添加task的配置，而在SpringBoot项目中一般使用config配置类的方式添加配置，所以新建一个 AsyncConfig 类
 ...
 * 使用 @Scheduled(fixedDelay = 5000) 注解 ：上一次执行完毕时间点之后 5 秒再执行（fixedDelay 属性：定时任务执行完成后再次执行定时任务的延时（需等待上次定时任务完成），
 * 单位毫秒）使用 @Scheduled(initialDelay = 1000, fixedRate = 3000) 注解 ：第一次延迟1秒后执行，之后按 fixedRate 的规则每 3 秒执行
